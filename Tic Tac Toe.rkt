...
;Function that returns the type, number, quantity of tokens to win, line which is closest to win, and blank spaces
...
(define (checkViability_aux player1 player2 gamep1 gamep2 avaipositions matrix)
  (cond
    ...
    ((and (not (null? gamep2)) (equal? (length (getAt 5 gamep2)) 1)) (getAt 1 (getAt 5 gamep2)))
...
;Function that returns the elements of intersection between two lists
...
(define (pretty-print-game board condition)
  (for ([i (length board)])
...
;Animación cuando el jugador pierde
  ...
  (define (leer matrizc)
    ...
    (cond ((equal? (car Matrix) "No movements left to win")
           (for-each
            (lambda (row rowc) (leer-aux row rowc)) (cdr Matrix) matrizc)
           )
           
          (else
           (for-each
            (lambda (row rowc) (leer-aux row rowc)) Matrix matrizc)
           )
          )
  ...
  (define Matrix (generateMatrix rows cols))
  (define Enable #t)
...
(define (game_aux player1 player2 matrix turn)
  (cond
    ((equal? (car matrix) "No movements left to win") (pretty-print-game (cdr matrix) "Tie"))
    ((checkVictory player1 matrix) (pretty-print-game matrix "Player wins"))
    ((checkVictory player2 matrix) (pretty-print-game  matrix "Computer wins"))
    (else
     (pretty-print-game matrix "InProgress")
     (cond
       ((zero? turn)
